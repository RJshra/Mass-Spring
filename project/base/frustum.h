...
public:
	...
	bool intersect(const BoundingBox& aabb, const glm::mat4& modelMatrix) const {
		...
		center = modelMatrix * glm::vec4(center, 1.0f);
		glm::vec3 forward = planes[NearFace].normal * extents.z;
		glm::vec3 right = glm::cross(planes[TopFace].normal,planes[NearFace].normal) * extents.x;
		glm::vec3 up = glm::cross(planes[NearFace].normal, planes[RightFace].normal) * extents.y;
		
		const float newIi = std::fabs(glm::dot(glm::vec3{ 1.f, 0.f, 0.f }, right)) +
			std::fabs(glm::dot(glm::vec3{ 1.f, 0.f, 0.f }, up)) +
			std::fabs(glm::dot(glm::vec3{ 1.f, 0.f, 0.f }, forward));

		const float newIj = std::fabs(glm::dot(glm::vec3{ 0.f, 1.f, 0.f }, right)) +
			std::fabs(glm::dot(glm::vec3{ 0.f, 1.f, 0.f }, up)) +
			std::fabs(glm::dot(glm::vec3{ 0.f, 1.f, 0.f }, forward));

		const float newIk = std::fabs(glm::dot(glm::vec3{ 0.f, 0.f, 1.f }, right)) +
			std::fabs(glm::dot(glm::vec3{ 0.f, 0.f, 1.f }, up)) +
			std::fabs(glm::dot(glm::vec3{ 0.f, 0.f, 1.f }, forward));

		extents = { newIi,newIj,newIk };
		for (int i = 0; i < 6; i++) {
			float r = extents.x * fabs(planes[i].normal.x) + extents.y * fabs(planes[i].normal.y)
				+ extents.z * fabs(planes[i].normal.z);
			if (-r > planes[i].getSignedDistanceToPoint(center))
				return false;
		}
		return true;*/
		glm::vec3 center = (aabb.max + aabb.min) * 0.5f;
