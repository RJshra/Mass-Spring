...
MassSpring::MassSpring(const Options& options): Application(options) {
	load_data(filepath);
...
void MassSpring::constructFunction() {
	printf("1");
...
void MassSpring::load_data(const char* filepath) {
	std::ifstream file;
	file.open(filepath);
	if (file.fail()) {
		file.close();
